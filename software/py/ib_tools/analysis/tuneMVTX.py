...
ladders = params['_cable_resistance']
feeids = sorted([(int(lstr[1]) << 12) | int(lstr[3:]) | (gbt << 8) for lstr in ladders for gbt in range(3)])
...
for feeid in stave_feeids:
    ...
    layer = feeid >> 12
    stave = feeid & 0x1F
    staveid = f'L{layer:01d}_{stave:02d}'
    config[staveid] = {}
    for chip in range(9):
        chipid = f'CHIP_{chip}'
        #if staveid+chipid not in ["L0_01CHIP_8","L1_03CHIP_5","L1_03CHIP_8"]: continue
        axis = axes[row-1][chip]
        axis.grid()
        if (chip == 0):
            axis.set_ylabel(f"{staveid}")
        if (row == 8):
            axis.set_xlabel(f"chip {chip}")
        for ithr in reversed(ithrs):
            ...
            axis.scatter(x, y, label=f'ITHR={ithr}')
