...
static inline void map_bad_pixels(float* thrs, size_t n, string& prefix, uint32_t feeId){
    ...
    // save bad pixels if any
    if (nbad > 0) {
        std::cout << "Found " << nbad << " bad pixels" << std::endl;
        ostringstream fname;
        fname << prefix << ( (prefix != "") ? "_" : "") << "badpixel_map_" << feeId << ".dat";
        ofstream f(fname.str().c_str(), ios::trunc);
        f.write(reinterpret_cast<const char*>(badpix), nbad * sizeof(uint32_t));
        f.close();
    }
    _mm_free(badpix);
                ...
                const int& n_vcasn_ithr = 1)
{
  ...
  if (! decoder->isThrTuning) {
    ...
    map_bad_pixels(thrs,3*1025*512,prefix,decoder->feeid);
