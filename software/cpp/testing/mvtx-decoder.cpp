...
static inline void map_bad_pixels(float*& thrs, size_t n, string& prefix, int feeId){
  uint32_t* badpix  = reinterpret_cast<uint32_t*>(_mm_malloc( n*sizeof(uint32_t), 4096) );
  size_t nbad = 0;
        int counter =0;
    for (size_t i = 0; i < n; i++) {
        // check if value of pixel is less than or equal to 0.0
        float val = thrs[i];
        if (val <= 0.0) {
                //std::cout << thrs[i] << std::endl;
            // encode pixel position as uint32_t
            int x = i % 3072;  // Assuming the image width is 3072
            int y = i / 3072;  // Assuming the image width is 3072
            uint32_t id = (x << 9) | y;  // Combine x and y using bit manipulation
                counter++;
            // add pixel ID to bad pixel list
            badpix[nbad++] = id;
            //nbad++;
        }
    }
  ...
  ostringstream fname;
  fname << prefix << ( (prefix != "") ? "_" : "") << "bad_pixel_map_" << feeId << ".dat";
  std::cout<<fname.str().c_str()<<std::endl;
	if(nbad > 0){
    std::cout<< "Found " << nbad << " bad pixels in FEE " << feeId << std::endl;
    ofstream fout(fname.str().c_str(), ios_base::trunc);
    fout.write(reinterpret_cast<const char*>(badpix), nbad*sizeof(uint32_t));
    fout.close();
  }
  std:: cout << "Found " << nbad << " bad pixels in FEE " << feeId << std::endl;
  _mm_free(badpix);
                ...
                const int& n_vcasn_ithr = 1)
{
  ...
  if (! decoder->isThrTuning) {
    ...
    map_bad_pixels(thrs,nChipsPerLane*1024*n_row,prefix,decoder->feeid);
    save_file(fname.str(),n_row,nChipsPerLane,thrs);
    fname.str("");
    fname.clear();
...
// void event_hitmap_for_feeid(struct decoder_t* decoder, string& prefix){

// }


// void run_event_hitmap(struct decoder_t* decoder, string& prefix)
// {

//   // find all feeids
//   get_all_feeids(decoder);
//   for (auto& _feeid : decoder->feeids)
//   {
//     uint32_t* hitmap = reinterpret_cast<uint32_t*>(_mm_malloc(3 * 1024 * 512 * sizeof(uint32_t), 4096));
//     bzero(hitmap, 3 * 1024 * 512 * sizeof(uint32_t));
//     decoder->feeid = _feeid;
//     event_hitmap_for_feeid(decoder,
//   }

//   std::cout << "Runing Event hit analysis for feeid " << decoder->feeid << "..." << std::endl;
//   uint32_t print_hb_cnt = 1000;
//   progressbar bar(100);
//   ostringstream ss;
//   ss << " out of " << (print_hb_cnt * 100) << " events.";
